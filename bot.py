...
def parse_price(v):
    if not v: return 0
    t = str(v).upper().strip()
    # Ignore les valeurs non-prix
    if t in ("FREE","N/A","—","-","","0","NO PRICE","NULL","NONE"): return 0
    t = t.replace(",","").replace(" ","").replace("\xa0","").replace("'","").replace(".","")
    try:
        if "M" in t:
            num = t.replace("M","").replace("\xa0","")
        ...
        # Ignore les prix aberrants (trop petits = Free, trop grands = bug)
        if val < 200 or val > 50_000_000: return 0
        return val
    except: return 0
        ...
        if html and "Just a moment" not in html and "Enable JavaScript" not in html:
            ...
            for row in rows[:60]:
                try:
                    ...
                    # Extrait prix valides (ignore 0, Free, aberrants)
                    all_p = []
                    for col in cols:
                        txt = col.get_text(strip=True)
                        # Cherche aussi les spans avec data-price
                        span = col.select_one("[data-price],[data-val]")
                        if span:
                            txt = span.get("data-price") or span.get("data-val") or txt
                        p = parse_price(txt)
                        if p >= 200:
                            all_p.append(p)
                    all_p = sorted(set(all_p))  # déduplique et trie
                    if len(all_p) < 1: continue
                    # Si un seul prix, snipe = 92% du marché (estimation)
                    market = all_p[-1]
...
@tree.command(name="meilleursfiltre", description="🧠 Analyse TOUT le marché et trouve les meilleurs filtres automatiquement")
async def cmd_meilleursfiltre(interaction: discord.Interaction):
    await interaction.response.defer()
    pl = get_plat(interaction.user.id)

    embed_wait = discord.Embed(
        title="🧠 Analyse complète du marché FC26 en cours...",
        description="Je scanne toutes les positions, ligues et tranches de prix. Cela peut prendre 30-60 secondes.",
        color=0xFEE75C
    )
    await interaction.followup.send(embed=embed_wait)

    # Scans multiples en parallèle sur différents filtres
    scan_configs = [
        ("🚨 Erreurs Prix",     {"min_discount": 20}),
        ("⚡ Snipe 50K",        {"max_price": 50000}),
        ("⚡ Snipe 100K",       {"max_price": 100000}),
        ("💎 Premium",          {"min_price": 100000}),
        ("🔥 Meta 87+",         {"min_rating": 87}),
        ("🔥 Meta 90+",         {"min_rating": 90}),
        ("💰 Budget",           {"max_price": 15000}),
        ("📍 Attaquants",       {"position": "ST"}),
        ("📍 Milieux",          {"position": "CAM"}),
        ("📍 Défenseurs",       {"position": "CB"}),
        ("📍 Gardiens",         {"position": "GK"}),
    ]

    all_results = []
    for label, filt in scan_configs:
        try:
            players = await scrape_futbin(filt, pl)
            for p in players:
                p["filtre"] = label
                all_results.append(p)
            await asyncio.sleep(2)  # anti rate-limit
        except Exception as e:
            log.debug(f"filtre {label}: {e}")

    if not all_results:
        await interaction.followup.send(embed=discord.Embed(
            title="❌ Aucun résultat",
            description="Futbin est temporairement bloqué. Réessaie dans 1-2 minutes.",
            color=0xED4245
        ))
        return
    ...
    plat = PLATFORMS[pl]
    recap = discord.Embed(
        title=f"🧠 Analyse Marché FC26 — {len(unique)} opportunités uniques",
        description=f"Scan de {len(scan_configs)} filtres · Top résultats {plat['emoji']} {plat['label']}",
        color=0x5865F2,
    ...
    for p in unique:
        f = p.get("filtre","?")
        if f not in by_filtre: by_filtre[f] = []
        if len(by_filtre[f]) < 2: by_filtre[f].append(p)

    for label, players_f in list(by_filtre.items())[:6]:
        if not players_f: continue
        lines = []
        for p in players_f:
            lines.append(f"• **{p['name']}** {p['rating']} {p['position']} · 🎯{fmt(p['snipe'])} → +{fmt(p['profit'])} ({p['roi']}% ROI)")
        recap.add_field(name=label, value="\n".join(lines), inline=False)

    best = unique[0]
    recap.add_field(
        name="🏆 MEILLEURE AFFAIRE GLOBALE",
        value=f"**{best['name']}** {best['rating']} {best['position']} | Snipe: {fmt(best['snipe'])} -> +{fmt(best['profit'])} coins ({best['roi']}% ROI)",
        inline=False
    )
    recap.set_footer(text=f"FC26 Bot · {len(unique)} opportunités · Taxe EA 5%")
    await interaction.followup.send(embed=recap)

    # Envoie les 5 meilleures cartes détaillées
    await interaction.followup.send("**🔥 Top 5 meilleures affaires détaillées :**")
    for p in unique[:5]:
        await interaction.followup.send(embed=alert_embed(p, pl))
...
async def cmd_top(interaction: discord.Interaction, plateforme: str = None):
    ...
    plat = PLATFORMS[pl]
    embed = discord.Embed(
        title=f"🏆 Top 10 Meilleures Affaires {plat['emoji']} {plat['label']}",
        color=plat["color"], timestamp=datetime.utcnow()
    )
    lines = []
    for i,p in enumerate(unique[:10],1):
        medal = "🥇" if i==1 else ("🥈" if i==2 else ("🥉" if i==3 else f"{i}."))
        lines.append(f"{medal} **{p['name']}** {p['rating']} {p['position']} · 🎯{fmt(p['snipe'])} → +{fmt(p['profit'])} ({p['roi']}%)")
    embed.add_field(name="Classement", value="\n".join(lines) if lines else "Aucun résultat", inline=False)
    embed.set_footer(text="FC26 Bot · Taxe EA 5% incluse")
